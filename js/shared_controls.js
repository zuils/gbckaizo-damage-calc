...
$(".set-selector").change(function () {
	...
	if (pokemon) {
		...
		if (regSets || randset) {
			...
			for (i = 0; i < LEGACY_STATS[gen].length; i++) {
				// console.log(set.ivs[LEGACY_STATS[gen][i]]);
...
function createField() {
	...
	var createSide = function (i) {
		return new calc.Side({
			...
			isTailwind: isTailwind[i], isHelpingHand: isHelpingHand[i], isFriendGuard: isFriendGuard[i], isBadgeAtk: isBadgeAtk[i], isBadgeSpec: isBadgeSpec[i], isBadgeDef: isBadgeDef[i], isBadgeSpeed: isBadgeSpeed[i], isBadgeBoosted: isBadgeBoosted[i],
